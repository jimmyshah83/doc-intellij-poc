...
doc_intelligence_client = DocumentAnalysisClient(doc_intelligence_endpoint, AzureKeyCredential(doc_intelligence_key))

# Start recognizing forms
poller = doc_intelligence_client.begin_analyze_document_from_url("prebuilt-document", formUrl)
...
for kv_pair in result.key_value_pairs:
    if kv_pair.key and kv_pair.value:
        field = Field(kv_pair.key.content, kv_pair.value.content, kv_pair.confidence)
    else:
...
for field in fields:
    print("Key '{}': Value: '{}', Confidence: {}".format(field.key, field.value, field.confidence))
